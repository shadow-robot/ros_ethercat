...
EtherCAT_AL::scan_slaves(void)
{
  ...
  const uint16_t SII_datalen = EC_Slave_RD[SII_ControlStatus].size + EC_Slave_RD[SII_Address].size + EC_Slave_RD[SII_Data].size;
  ...
  for (unsigned int i = 0; i < m_num_slaves; i++)
   ...
   nw2host(data+EC_Slave_RD[SII_ControlStatus].size + EC_Slave_RD[SII_Address].size,serial);
	...
	ec_log(EC_LOG_INFO, "AL creating SlaveHandler: pos=%d, adr=0x%x, Prod. Code=0x%x, rev=0x%x, Serial=%d\n", 
	       adp2ringpos(adp),(uint16_t) sconf->get_station_address(),productcode,revision,serial);
      }
		      ...
		      unsigned char * a_buffer)
{
  ...
  const uint16_t SII_control_datalen = EC_Slave_RD[SII_ControlStatus].size + EC_Slave_RD[SII_Address].size;
