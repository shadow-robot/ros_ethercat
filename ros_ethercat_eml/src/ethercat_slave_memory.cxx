...
EC_SyncMan::dump(unsigned char * a_buffer) const
{
  ...
  uint8_t thirdbyte = BufferType | (Direction << 2) | ((uint8_t) ECATEventEnable << 4) | ((uint8_t) ALEventEnable << 5) | ((uint8_t) WatchdogEnable << 6);
  a_buffer = host2nw(a_buffer,thirdbyte);
  uint8_t fourthbyte = (uint8_t) WriteEvent | ((uint8_t) ReadEvent << 1) | ((uint8_t) WatchdogTrigger << 2) | ((uint8_t) QueuedState << 3) | (BufferedState << 4);
